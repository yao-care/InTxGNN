...
# 印度 InTxGNN 設定
BASE_URL = "https://intxgnn.yao.care"
...
def generate_capability_statement() -> dict:
    ...
    return {
        ...
        "id": "intxgnn-fhir-server",
        "url": f"{BASE_URL}/fhir/metadata",
        "version": "1.0.0",
        "name": "InTxGNNFHIRServer",
        "title": "InTxGNN FHIR Server - Drug Repurposing Predictions for India",
        "status": "active",
        "date": datetime.now().strftime("%Y-%m-%d"),
        "publisher": "InTxGNN Project",
        "description": "India CDSCO Drug Repurposing Prediction System - FHIR API",
        "kind": "instance",
        "fhirVersion": "4.0.1",
        "format": ["json"],
        "rest": [{
...
def generate_medication_knowledge(drug_id: str, drug_name: str, evidence_level: str = "L5") -> dict:
    ...
    return {
        ...
        "id": slug,
        "meta": {
            "profile": [f"{BASE_URL}/fhir/StructureDefinition/InTxGNNMedicationKnowledge"]
        },
        "status": "active",
        "code": {
            "coding": [{
...
def generate_clinical_use_definition(
    ...
    return {
        ...
        "id": resource_id,
        "meta": {
            "profile": [f"{BASE_URL}/fhir/StructureDefinition/InTxGNNClinicalUseDefinition"]
        },
        "type": "indication",
...
def main():
    ...
    for drug_id in drugs:
        ...
        if "drug_ingredient" in drug_rows.columns:
            drug_name = drug_rows["drug_ingredient"].iloc[0] or drug_id
