---
layout: default
title: "heart disease (cardiovascular) News"
parent: Health News
nav_exclude: true
description: "Health news about heart disease (cardiovascular). 2 articles, 31 related drugs."
permalink: /news/cardiovascular/
---

# heart disease (cardiovascular) News

[← Back to News Overview]({{ '/news/' | relative_url }})

---

<p class="key-answer" data-question="What news is there about heart disease (cardiovascular)?">
<strong>heart disease (cardiovascular)</strong> currently has <strong>2 news articles</strong> and 31 related drugs.
</p>

<div class="key-takeaway">
This page brings together the latest health news about “heart disease” and lists the drugs in the InTxGNN database whose predicted indications include this disease.
</div>

<div class="related-drugs-card">
<strong>Related drug reports</strong>
<p>The predicted indications of the following drugs may be related to this disease:</p>
<ul>
<li><a href="{{ '/drugs/alteplase/' | relative_url }}">Alteplase</a></li>
<li><a href="{{ '/drugs/atosiban/' | relative_url }}">Atosiban</a></li>
<li><a href="{{ '/drugs/cyclandelate/' | relative_url }}">Cyclandelate</a></li>
<li><a href="{{ '/drugs/cyproterone_acetate/' | relative_url }}">Cyproterone Acetate</a></li>
<li><a href="{{ '/drugs/db00009/' | relative_url }}">ALTEPLASE</a></li>
<li><a href="{{ '/drugs/db00015/' | relative_url }}">RETEPLASE</a></li>
<li><a href="{{ '/drugs/db00031/' | relative_url }}">TENECTEPLASE</a></li>
<li><a href="{{ '/drugs/db00063/' | relative_url }}">EPTIFIBATIDE</a></li>
<li><a href="{{ '/drugs/db00175/' | relative_url }}">PRAVASTATIN</a></li>
<li><a href="{{ '/drugs/db00227/' | relative_url }}">LOVASTATIN</a></li>
<li><a href="{{ '/drugs/db00335/' | relative_url }}">ATENOLOL</a></li>
<li><a href="{{ '/drugs/db00641/' | relative_url }}">SIMVASTATIN</a></li>
<li><a href="{{ '/drugs/db00661/' | relative_url }}">VERAPAMIL</a></li>
<li><a href="{{ '/drugs/db00722/' | relative_url }}">LISINOPRIL</a></li>
<li><a href="{{ '/drugs/db00758/' | relative_url }}">CLOPIDOGREL</a></li>
<li><a href="{{ '/drugs/db00775/' | relative_url }}">TIROFIBAN</a></li>
<li><a href="{{ '/drugs/db00806/' | relative_url }}">PENTOXIFYLLINE</a></li>
<li><a href="{{ '/drugs/db01076/' | relative_url }}">ATORVASTATIN</a></li>
<li><a href="{{ '/drugs/db01095/' | relative_url }}">FLUVASTATIN</a></li>
<li><a href="{{ '/drugs/db01098/' | relative_url }}">ROSUVASTATIN</a></li>
<li><a href="{{ '/drugs/db01113/' | relative_url }}">PAPAVERINE</a></li>
<li><a href="{{ '/drugs/db06209/' | relative_url }}">PRASUGREL</a></li>
<li><a href="{{ '/drugs/db08816/' | relative_url }}">TICAGRELOR</a></li>
<li><a href="{{ '/drugs/db08941/' | relative_url }}">ISOXSUPRINE</a></li>
<li><a href="{{ '/drugs/doxapram/' | relative_url }}">Doxapram</a></li>
<li><a href="{{ '/drugs/ebastine/' | relative_url }}">Ebastine</a></li>
<li><a href="{{ '/drugs/isosorbide_dinitrate/' | relative_url }}">Isosorbide Dinitrate</a></li>
<li><a href="{{ '/drugs/isoxsuprine/' | relative_url }}">Isoxsuprine</a></li>
<li><a href="{{ '/drugs/lubiprostone/' | relative_url }}">Lubiprostone</a></li>
<li><a href="{{ '/drugs/propafenone/' | relative_url }}">Propafenone</a></li>
<li><a href="{{ '/drugs/streptokinase/' | relative_url }}">Streptokinase</a></li>
</ul>
</div>

## Related News (2)

### [How are noisy celebrations and DJs affecting our hearing? - The Hindu](https://news.google.?oc=5)

2026-09-19

Source: [The Hindu](https://news.google.?oc=5)

---

### [Midlife Onset of Increasing Pulse Pressure Not Linked to Menopause - The Cardiology Advisor](https://news.google.?oc=5)

2026-09-18

Source: [The Cardiology Advisor](https://news.google.?oc=5)

---


<div class="disclaimer">
<strong>Disclaimer</strong>: The news on this page is collected automatically and is for research reference only; it does not constitute medical advice.
</div>
